...
- 보직명(직급) 형태 퇴직 표 파싱 (예: `교육정책국장(장학관)`)
- 파싱 결과 미리보기 테이블에서 셀 직접 수정 (관리자 전용)
- 검수 후 일괄 저장 / JSON 파일 가져오기 (관리자 전용)
- 저장 데이터 목록 조회 및 검색 (모든 사용자)
- 이름 / 기관 / 과목 / 현임기관 / 발령일 부분 일치 검색
- 인라인 수정·삭제 (관리자 전용)
- 페이지네이션 및 컬럼 정렬
- 관리자: 본인 비밀번호 변경 모달
- 관리자: 계정 관리 패널에서 사용자 비밀번호 초기화
...
```
직위(급) 성 명 현 임 기 관
교육정책국장 (장학관) 김 덕 주 대 전 광 역 시 교 육 청
교 육 정 책 국
교육공무원법 제47조의 규정에 의하여 2012년 8월 31일자로 정년퇴직.
2012. 8. 31.
```

- `organization` 필드에 `"정년퇴직"` / `"명예퇴직"` / `"의원면직"` 저장
- 현임기관이 다음 행으로 이어지는 경우 자동 합산
- `position` 필드: `"교육정책국장(장학관)"` 형태로 저장
...
| PUT | `/api/auth/password` | 로그인 | 본인 비밀번호 변경 (현재 비밀번호 확인 후 변경, 토큰 즉시 무효화) |
| GET | `/api/auth/users` | 관리자 | 전체 사용자 목록 조회 |
| PUT | `/api/auth/users/:username/password` | 관리자 | 지정 사용자 비밀번호 초기화 |
